...
sage_comm <- read_json("../synapseAnnotations/data/sageCommunity.json", simplifyVector = FALSE)

definitions <- map(sage_comm, create_schema_from_entry) %>%
  list("$schema" = "http://json-schema.org/draft-07/schema#",
       "$id" = "http://example.com/definitions.json",
       "definitions" = .) %>%
  toJSON(auto_unbox = TRUE, pretty = TRUE)

write(definitions, "../synapseAnnotations/schemas/definitions.json")
